...
var next_weapon: WeaponSlotP

#The List of All Available weapons in the game
var spray_profiles: Dictionary = {}
var _count = 0
var shot_tween
@export var weapon_stack:Array[WeaponSlotP] #An Array of weapons currently in possesion by the player
var current_weapon_slot: WeaponSlotP = null
...
func initialize(_weapon_slot: WeaponSlotP):
...
func exit(_next_weapon: WeaponSlotP) -> void:
...
func change_weapon(weapon_slot: WeaponSlotP) -> void:
...
func shoot() -> void:
	if current_weapon_slot.current_ammo != 0 or not current_weapon_slot.weapon.has_ammo:
		...
		if not animation_player.is_playing():
			var reloadTime = 1/GlobalGame.player.gun_reload_time_multiplier
			if GlobalGame.player.beserk_reload_is_on and GlobalGame.player.health_is_low():
				reloadTime /= 1.2
				
			animation_player.play(current_weapon_slot.weapon.shoot_animation, -1, reloadTime)
			...
			var sfxp = AudioManager.play_sfx("shoot", 0.7, 1.1, shooter)
			
			print("upgrade user quiet shot",user.quiet_shot)
			if user.quiet_shot:
				sfxp.volume_db -= 15
				print("upgrade quiet")
...
func add_flashlight_light():
	await get_tree().create_timer(1).timeout
	var tween = get_tree().create_tween()
	tween.tween_property(flashlight, "light_energy", 1, .4)
	
func remove_flashlight_ligth():
	var tween = get_tree().create_tween()
	tween.tween_property(flashlight, "light_energy", 0, .1)
	tween.tween_callback(add_flashlight_light)
...
func load_projectile(_spread):
	...
	_projectile._Set_Projectile(current_weapon_slot.weapon.damage,_spread,current_weapon_slot.weapon.fire_range, bullet_point_origin, user)
...
func melee() -> void:
	...
	if Current_Anim != current_weapon_slot.weapon.melee_animation:
		...
		print(melee_hitbox.is_colliding(),"MELEE?")
		
		melee_hitbox.scale *= owner.melee_range_multiplier
		if melee_hitbox.is_colliding():
			var colliders = melee_hitbox.get_collision_count()
			print(colliders,"MELEE!")
			for c in colliders:
				var Target = melee_hitbox.get_collider(c)
				print("melee colliding",Target)
				if  Target.has_method("Hit_Successful") and Target != owner:
					user.shakeCamera()
					
					AudioManager.play_sfx("punch_%s" % randi_range(1, 3))
					...
					var damage = current_weapon_slot.weapon.melee_damage*owner.melee_damage_multiplier
					if not owner.is_on_floor():
						damage *= owner.damage_in_air_multiplier
					if owner.beserk_damage_is_on and owner.health_iS_low():
						damage *= 1.5
					Target.Hit_Successful(damage, Direction, Position, parent, 25*owner.melee_knockback_multiplier)
		
		melee_hitbox.scale *= owner.melee_range_multiplier
		
func drop(_slot: WeaponSlotP) -> void:
...
func _on_pick_up_detection_body_entered(body: RigidBody3D):
	return pickup_ammo(body.weapon)

func pickup_ammo(weapon, body = null):
	...
	for slot in weapon_stack:
		if slot.weapon == weapon_slot.weapon:
			...
			weapon_slot.current_ammo = remaining#max(remaining - weapon_slot.current_ammo,0)
