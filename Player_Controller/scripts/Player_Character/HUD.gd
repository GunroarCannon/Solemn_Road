...
@onready var current_health_label = $debug_hud/HBoxContainer4/CurrentHealth

@onready var bulletLabel: Label=  $StatsMarginContainer/VBoxContainer/Bullets/Label
@onready var healthLabel: Label =  $StatsMarginContainer/VBoxContainer/Health/Label
...
func _on_weapons_manager_update_ammo(Ammo):
	...
	bulletLabel.set_text("X"+str(Ammo[0]))
...
func _on_weapons_manager_connect_weapon_to_hud(_weapon_resouce: WeaponResourceP):
	_weapon_resouce.update_overlay.connect(load_over_lay_texture)

func _on_hit(health : float):
	current_health_label.set_text(str(health))
