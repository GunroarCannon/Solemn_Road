...
@onready var Explosion = preload("res://explosion_1.tscn")
...
func _Set_Projectile(_damage: int = 0,_spread:Vector2 = Vector2.ZERO, _Range: int = 1000, origin_point: Vector3 = Vector3.ZERO, user : CharacterBody3D = null):
	damage = _damage * user.gun_damage_multiplier
	
	if not user.is_on_floor():
		damage *= user.damage_in_air_multiplier
	
	if user.beserk_damage_is_on and user.health_iS_low():
		damage *= 1.5
		
	Fire_Projectile(_spread, _Range, Rigid_Body_Projectile, origin_point, user)

func Fire_Projectile(_spread: Vector2 ,_range: int, _proj:PackedScene, origin_point: Vector3, user):
	var Camera_Collision = Camera_Ray_Cast(_spread,_range)
	
	match Projectile_Type:
		"Hitscan":
			Hit_Scan_Collision(Camera_Collision, damage, origin_point, user)
...
func Hit_Scan_Collision(Collision: Array,_damage: float, origin_point: Vector3, user : CharacterBody3D):
	...
	if Collision[0]:
		...
		if Collision[0].is_in_group("Target"):
			...
			if Bullet_Collision:
				Hit_Scan_damage(Bullet_Collision.collider, Bullet_Direction,Bullet_Collision.position,_damage,user)
				if pass_through and check_pass_through(Bullet_Collision.collider, Bullet_Collision.rid):
					...
					Hit_Scan_Collision(pass_through_collision,pass_through_damage,Bullet_Collision.position,user)
					return
			queue_free()
...
func Hit_Scan_damage(Collider, Direction, Position, _damage, user):
	if Collider.is_in_group("Target") and Collider.has_method("Hit_Successful"):
		...
		#expo.scale *= .2
		#
		#Collider.add_child(expo)
		#expo.lifetime =1.4
		#expo.global_position = Position
		
		#add_child(expo)
		Hit_Successfull.emit()
		print(user)
		Collider.Hit_Successful(_damage, Direction, Position, user, min(15, _damage) * 5 * user.gun_knockback_multiplier)
...
func _on_body_entered(body, _proj, _norm):
	if body.is_in_group("Target") && body.has_method("Hit_Successful"):
		body.Hit_Successful(damage,null,null,owner)
		Hit_Successfull.emit()
