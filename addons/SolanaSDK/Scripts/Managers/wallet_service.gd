...
func log_in_success() -> void:
	...
	var address = get_pubkey().to_string()
	print("Logged in as: ", address)
	
	# 1. Get balance (This function returns SOL as a float, e.g., 0.5)
	var balance_sol = await SolanaService.get_balance(address)
	print("Current Balance: ", balance_sol, " SOL")
	
	# 2. Check if balance is low (less than 0.1 SOL)
	if balance_sol < 0.1:
		print("Tester is broke! Requesting Devnet Airdrop...")
		
		# request_airdrop usually takes Lamports. Let's ask for 1 SOL.
		var signature = await SolanaService.get_airdrop(address, 1_000_000_000)
		
		if signature != "":
			print("Airdrop successful! Signature: ", signature)
			# Optional: Wait a second and refresh UI balance
			await get_tree().create_timer(1.0).timeout
			var new_balance = await SolanaService.get_balance(address)
			print("New Balance: ", new_balance)
		else:
			print("Airdrop failed. Faucet might be rate-limited.")
	else:
		print("Tester has enough SOL to play.")
	if done_wallet:
		print("doning wallet")
		done_wallet.done_wallet(true)
