...
func _physics_process(delta: float):
	...
	# 3. Target Logic
	#print(chasing, target_enemy)
	
	
	var increase_speed =randi_range(0,100)<70
	
	if increase_speed and target_enemy:
		speed_increased = target_enemy.global_position
		
	if target_enemy and target_enemy.is_dead:
		target_enemy = null;
		
	if chasing and is_instance_valid(target_enemy):# Calculate distance to player
		if last_attacked <= 0:
			var pos = target_enemy.global_position
			if speed_increased:
				pass#pos = speed_increased
			...
			if global_position.distance_to(pos) < 2.2:
				attack()
	...
	#why is next path position always the same but target position is very far
	var direction = (next_path_pos - global_position).normalized()
	print(global_position, nav_agent.target_position, chasing, speed_increased, "walker heyyyyyyyyy",direction)
	...
	#current_speed = 0 if anim.current_animation == "hit" else speed
	
	current_speed = current_speed if increase_speed else speed * 50
	#print(direction,speed_increased)
	# Check if the body actually moved this frame
	...
	var is_stationary = in_same_pos > 0.5
	print(nav_agent.path_desired_distance)
	nav_agent.path_desired_distance = 0.2
	if is_stationary:
		velocity += Vector3(randf(), 0, randf()).normalized() * 0.5
		in_same_pos = 0 # Reset timer after the "nudge"
		speed_increased = null
		
	
	var move_velocity = direction * current_speed
	print("walker ",move_velocity)
...
func update_animations():
	...
	var is_busy = do_not_interrupt() and anim.is_playing()
...
func do_not_interrupt():
	return no_interrupt.has(anim.current_animation)
	
func attack():
	speed_increased = null
	if do_not_interrupt(): return
	if anim.current_animation == "kick": return
...
func get_hit(damage, bullet = null, knockback_force = 15.0, enemy = null):
	...
	knockback_velocity = knockback_dir * knockback_force * .65
	print("Knockback vel is",knockback_velocity, knockback_force,knockback_dir)

func die():
	velocity = Vector3.ZERO
	play_animation("fly_away", true)
...
func play_animation(name : String, force_play : bool = false):
	if is_dead: return
	
	if anim.current_animation == name: return
	
	print("Tryinig to play animation", name)
	
	if not force_play and old_animation == "hit":
		print("changed to standn")
		name = "stand_up"
	
	print(anim.current_animation)
		
	print("[pppppppppppppppppplaying animation", name)
	old_animation = name
	anim.play(name)
...
func _on_area_3d_body_entered(body: Node3D) -> void:
	...
	if body is CharacterBody3D and body != self and body == GlobalGame.player:
		aggro(body)
