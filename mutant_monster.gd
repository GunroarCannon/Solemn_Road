...
func check_footstep_sound(direction: Vector3, _delta):
	if direction.length() >= 0.9:
		...
		#_speed = 
		if footstep_timer >= .5*min(2,base_speed/(_speed/base_speed)):
			footstep_timer = 0
			play_footstep_sound()
...
func _ready():
	...
	AudioManager.loud_sound.connect(_on_loud_sound_heard)
	
	print(fall_gravity, "is fall gravity")
	calculate_movement_parameters()
	pick_random_city_spot()

func _on_loud_sound_heard(pos: Vector3):
	print("almost loud mut")
	if not chasing and global_position.distance_to(pos) < 20:
		nav_agent.target_position = pos
		time_stuck = -10
		heard_sound = true
		print("mut loud sound heard")
...
func _physics_process(delta: float):
	...
	var amp = 1
	if heard_sound and not chasing:
		amp = 3
	...
	check_footstep_sound(direction, delta)
...
func update_animations():
	...
	var is_busy = (current == "Death" or current == "Attack_1" or current == "Attack_2" or current == "Scream") and anim.is_playing()
	
	if is_busy:
		return
	
	if velocity.length() < 0.2:
		pass#play_animation("idle")
	else:
		if chasing:
...
func aggro(enemy : CharacterBody3D):
	...
	AudioManager.play_attached_sfx("loud_scream", self)
...
func attack():
	if anim.current_animation == "Attack_2": return
	
	play_animation("Attack_2")

func trigger_melee_check():
	AudioManager.play_attached_sfx("loud_pain", self)
	...
	if melee_box.is_colliding():
		for i in melee_box.get_collision_count():
			...
			if not collider.is_in_group("Enemy") and collider.has_method("Hit_Successful") and collider != self:
				...
				knockback_dir.y = 0
				knockback_velocity = knockback_dir * 20
...
func get_hit(damage, bullet = null, knockback_force = 15.0, enemy = null):
	...
	if health <= 0:
		...
		play_animation("Scream")
		AudioManager.play_attached_sfx("loud_pain", self, 0.5, 0.6)
